<?php
    ...
    if(isset($_POST['username']) && isset($_POST['password'])){
        ...
        $sql = "SELECT * FROM employeelist WHERE username = '$username' AND employee_status = 'Active'";
        ...
        if($result->num_rows > 0){
            ...
            if($realPassword == $password){
                ...
                if($status == 'in'){
                    ...
                    $sql = "SELECT * FROM attendance_csv WHERE employee_id = '$employee_id' and date = '$dateNow'";
                    $result = $con->query($sql);
                    ...
                    if($result->num_rows < 1){
                        ...
                        $sql = "SELECT * FROM attendance_csv WHERE employee_id = '$employee_id' and time_out = '00:00:00'";
                        $result = $con->query($sql);
                        ...
                        if($result->num_rows > 0) {
                            ...
                            $sql = "INSERT INTO `attendance_csv`(`employee_id`, `time_in`, `status`, `date`) VALUES ('$employee_id','$timeNow','$isLate','$dateNow')";
                ...
                // if time-out
                    ...
                    if($result->num_rows < 1){
                        ...
                        // titignan kung may attendance na hindi ka nakapag time out
                        $sql = "SELECT * FROM attendance_csv WHERE employee_id = '$employee_id' and time_out = NULL";
                        $result = $con->query($sql);
                        ...
                        if($result->num_rows > 0){
                            ...
                            if( $time_hr > $eligableWorkingHr ) {
                                $ot_hr = $time_hr - $eligableWorkingHr;
                                $time_hr = $eligableWorkingHr;

                                $sql = "INSERT INTO `overtime`(`employee_id`, `ot_hr`, `date`) VALUES ('$employee_id','$ot_hr','$date_created')";

                                $con->query($sql);
                            }

                            // updating to time out
                            $sql = "UPDATE `attendance_csv` SET `time_out`='$timeNow',`total_hr`=
                            '$time_hr', `ot_hr`='$ot_hr' WHERE id = '$attendance_id'";
                    ...
                    // if may nafetch na row meaning nag attendance ka na
                        ...
                        if($timed_out == NULL){
                            ...
                            if( $time_hr > $eligableWorkingHr) {
                                ...
                                $sql = "INSERT INTO `overtime`(`employee_id`, `ot_hr`, `date`) VALUES ('$employee_id','$ot_hr','$date')";
